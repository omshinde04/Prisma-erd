...
import {
  loadSchemaSource,
  SchemaSourceError,
} from "../src/services/schema-source-service.js";
...
test("createInitialParseResult returns empty normalized parse result", async () => {
  ...
  assert.equal(parseResult.metadata.modelCount, 0);
  assert.equal(parseResult.metadata.enumCount, 0);
  assert.equal(parseResult.metadata.relationCount, 0);
});

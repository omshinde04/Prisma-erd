...
async function timeAt(modelCount) {
  ...
  return { time: end - start, nodes: mappedGraph.nodes.length, edges: mappedGraph.edges.length };
}

for (const modelCount of [1000, 2000, 4000]) {
  const result = await timeAt(modelCount);
  console.log(`${modelCount} models (${result.nodes} nodes, ${result.edges} edges): ${result.time.toFixed(0)} ms`);
}
